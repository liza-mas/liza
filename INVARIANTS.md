...
| A terminal task's `acceptance_receipt` may leave live state for an immutable, content-addressed archive object: the task keeps exactly one digest reference per archived field, the object and its directory chain are durable before that reference is published, and inspection restores the value or fails explicitly. Task history is never archived | Unbounded live-state growth; evidence silently lost or overwritten | code (`task_archive.go`, `archive_acceptance_receipts.go`, `validate_task.go`), spec ([Archived Task Fields](specs/architecture/blackboard-schema.md#archived-task-fields)) |
