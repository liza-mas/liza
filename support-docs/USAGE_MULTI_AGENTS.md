...
forbids as edges, add `--awaits <id>[,<id>]` to either form, naming all of
them: the task wakes once they have all merged or one fails, instead of on
generated descendants or partial progress, while other dependency, blocker,
human-note and the task's own changes still wake it. The command rejects a
self-wait, unknown, settled or failed IDs, and a wait that would deadlock. A
later assessment without `--awaits` keeps the set minus merged tasks within the
same blocked episode; `--clear-awaits` drops it.
