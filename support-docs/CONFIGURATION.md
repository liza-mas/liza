...
- **After init:** Read with `§BRAND_BINARY_NAME§ config get config.post_worktree_cmd --json`; set with `§BRAND_BINARY_NAME§ config set config.post_worktree_cmd "your command" --json`. The general `get config.post_worktree_cmd` query remains supported. An identical value is a no-op; replacing a different value requires `--replace --reason "..."`. Comparison and mutation are atomic, so merge detection cannot overwrite an operator value committed first. The setter stores the command without running it and does not restart existing sessions. See [Changing worktree setup during a run](SUPPORT.md#changing-worktree-setup-during-a-run) for validation, authority, concurrency, and process-log-only audit semantics.
...
For an empty repository, continuing without a command is valid. After scaffolding merges,
an unambiguous supported layout may be detected automatically; otherwise the operator
validates and sets the command. There is no planner declaration or automatic configuration
of custom workflows. Bootstrap artifacts must be gitignored or already committed, and
a second run must change nothing observable. Git status must stay clean.
...
| `§BRAND_ENV_PREFIX§_AGENT_GENERATION` | For agent-authenticated lifecycle commands | -- | Registration-scoped fencing token supplied by `§BRAND_BINARY_NAME§ agent`; must match the caller's current registration. |
