...
An assessed task remains visible in total blocked counts, but unchanged blockers do not override another wake such as `PLANNING_COMPLETE`. Wake detection and rendered instructions use the same assessment-aware checks, including new dependency or descendant activity and targeted human notes. Independently of any blocked task, an operator note (`add-human-note <task-id|all>`) that no completed orchestrator turn has rendered wakes an idle orchestrator once with the `HUMAN_NOTE` trigger, ranked after `IMMEDIATE_DISCOVERY` and before `PLANNING_COMPLETE`; the `HUMAN_NOTE` turn renders every unseen note verbatim and, on success, stamps `orchestrator_seen_at` on the notes it rendered. A turn woken by a higher-ranked trigger stamps only the notes it consumed — those targeting a task (or `all`) that gained an assessment during the turn — so a blocked-task note is delivered once, not again as a `HUMAN_NOTE` request; a note whose request no turn completed is shown again on the next wake. Audit notes written by delete/recover commands are stamped at creation and never wake the orchestrator; audit notes recorded before this build are unstamped and appear once in the catch-up `HUMAN_NOTE` turn — they are records of past operations, not requests. The same rule applies to assessed hypothesis exhaustion. If status selects planning completion while a newly saved prompt still instructs blocked-task assessment, verify the engine build and restart the resident orchestrator supervisor after installing the repair; replacing the executable alone does not reload its prompt builder.
...
Read the recorded input with `§BRAND_BINARY_NAME§ get human_notes --json`.
The read-only result is an ordered list of `timestamp`, `message`, and `for`,
with `source` and `operation` when recorded. Select notes addressed to the task
or `all`; an empty history returns `[]`. Agents may read notes without operator
write authority. Reading notes does not acknowledge them or alter task state.
The orchestrator prompt requires this read before reassessment, including checks
...
**Symptom**: A reviewer role stops starting reviews while a reviewable task stays claimable. Earlier, `§BRAND_PROJECT_DIRNAME§/agent-outputs/supervisor-<role>-*.log` shows one error-level `Review claim error` and then nothing (further attempts are debug-level); a `reviewer_claim_circuit_open` anomaly names the task.
**Diagnosis**: `§BRAND_BINARY_NAME§ get anomalies --json` — the anomaly's `failure_class` says what must be repaired (`review_boundary_repair`, `acceptance_evidence` or `worktree_context`), `recovery` gives the command, and `attempts`, `first_failure`, `last_failure` and `cooldown_until` show the last recorded quarantine. After 3 identical failures, the work check excludes the candidate for 5 min, doubling per failed re-probe to 30 min. Claims prompted by other work still evaluate and remove the broken candidate before claiming a healthy one. These failures count even when the claim succeeds, but during cooldown they neither extend it nor update the durable anomaly.
**Fix**: Apply the `recovery` repair — for a broken review boundary, `§BRAND_BINARY_NAME§ update-review-commit <task-id> --changed-by <operator>`. A change to the task's status, `review_commit`, `base_commit`, `worktree` or history clears its key; so does successfully claiming that task. When only quarantined work remains, cooldown expiry permits a re-probe. No restart is needed: restarting loses the in-memory counters and re-attempts up to the threshold, while the durable anomaly remains deduplicated.

### Coding task blocked by an acceptance refusal
**Symptom**: A coding task goes from its initial (or rejected) status to `BLOCKED` without ever being claimed, with a `blocked_reason` starting `acceptance_evidence_invalid:` or `acceptance_claim_refused_repeatedly:`. The doer supervisor log shows `Refused claim escalated to BLOCKED for the orchestrator`.
**Diagnosis**: `§BRAND_BINARY_NAME§ get tasks <task-id> --json` — `blocked_reason` names the field, the allocation (`plan_ref`, or `spec_ref` when there is none) and the refusal; `plan_ref`, `spec_ref`, `validation` and `parent_tasks` are the inputs it was judged on. `acceptance_evidence_invalid` means the allocation cannot match the reviewed section (for example a slug instead of the section's heading text, or `validation` that differs from the reviewed commands). `acceptance_claim_refused_repeatedly` means three identical allocation refusals: usually no reviewed, merged planning parent allocates the task (or an approved proof drifted), but a persistent repository read failure looks the same.
**Fix**: The orchestrator corrects the allocation — normally `§BRAND_BINARY_NAME§ replace-task` with the reviewed values, or `§BRAND_BINARY_NAME§ reaffirm-proof` for a drifted proof — and, if the task itself is kept, `§BRAND_BINARY_NAME§ unblock-task <task-id>` restores it for a new claim. Unclassified refusals (a failed Git read of the carrier or a pinned reference) are not escalated and keep retrying.

### Abandoned background job
**Symptom**: `§BRAND_PROJECT_DIRNAME§/alerts.log` contains `ABANDONED BACKGROUND JOB`: an agent session ended while a background job it started was still running, and the provider killed the job. The supervisor kept the claim, so the task re-runs. Past the spinning restart threshold, `ABANDONED BACKGROUND JOB LOOP` follows and the task is `BLOCKED`.
**Diagnosis**: The alert names the agent, task, job id and job summary. Check what the job consumed or left half-applied: single-use fixtures, live-environment writes, a partial submission gate.
**Fix**: Replace spent inputs before the re-run. For a loop, find why the job cannot finish within the session before unblocking the task.
...
**Diagnosis**: `ls §BRAND_PROJECT_DIRNAME§/provider-quota-exhausted-*` or check `§BRAND_PROJECT_DIRNAME§/alerts.log` for `PROVIDER QUOTA EXHAUSTED`. The signal file records the provider's announced reset (`resets_at`, or `unknown`) and when the block lifts (`expires`). The block lifts one minute after the announced reset, but never sooner than five minutes after detection; with no announced reset, it lifts after 30 minutes. `PROVIDER QUOTA SPAWN BLOCKED` means a spawn was attempted before `expires`. An expired file no longer blocks anything. After the lift, the first agent turn on that provider acts as the probe: if the provider is still exhausted, the file is rewritten with the newly announced reset.
**Fix**: The block lifts on its own at `expires`, and pool auto-repair may then restore the doer and reviewer roles it covers, and a missing orchestrator. Other agents you started manually do not restart themselves: start them again once the block has lifted. To lift the block earlier, e.g. after switching accounts: `§BRAND_BINARY_NAME§ pause` then `§BRAND_BINARY_NAME§ resume`. Pause transitions RUNNING → PAUSED, and resume clears quota signals and restarts the sprint. Deleting the flag file also works. (`§BRAND_BINARY_NAME§ resume` alone fails because the system is still RUNNING, not PAUSED.)
