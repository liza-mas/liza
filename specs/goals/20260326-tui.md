...
An `IDLE` or `WAITING` agent whose role is held by a pause gate shows the gate instead (`⏸ CHECKPOINT`, `⏸ PAUSED`, `⏸ BREAKER`), derived from state with the supervisors' own predicate; an agent mid-turn keeps its status.
...
### Awaiting-Human Notice (persistent)

While the sprint is at `CHECKPOINT` or the system is `PAUSED`, a bar below the header names the
remedy, derived from state rather than from an alert, so it neither expires nor clears on keypress:

```
🚨 CHECKPOINT: agents paused; run "liza resume"
```

A transient `AWAITING HUMAN` banner is suppressed while the notice is shown.
