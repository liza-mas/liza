...
plan_ref: specs/plans/20260520-artifact-ref-protection/20260520-111957-architecture-2-code-planning-0.md

source_requirements: FR-001-17 through FR-001-22 and AC-001-14; architecture plan sections "Interfaces" and "Structural Decisions".
...
- Update the existing `MergeWorktree` call in `internal/ops/wt_merge.go` to pass `nil`.
...
source_requirements: FR-001-23 through FR-001-26, AC-001-11, and AC-001-12; architecture plan sections "Interfaces" and "Structural Decisions".
...
source_requirements: FR-001-27 and AC-001-13; architecture plan sections "Structural Decisions" and "Validation And Test Strategy".
...
```text
Task 1 has no depends_on field.
Task 2 depends_on: ["0"]
Task 3 depends_on: ["1"]
```
...
|------|-------|------------|
| `internal/ops/wt_merge.go` | Task 1, Task 2, Task 3 | Task 2 depends_on: ["0"]; Task 3 depends_on: ["1"] |
| `internal/ops/wt_merge_test.go` | Task 1, Task 2, Task 3 | Task 2 depends_on: ["0"]; Task 3 depends_on: ["1"] |
...
|---|-------------|--------|---------|--------|
| 1 | Guard must preserve existing CAS retry safety for concurrent merges. | NFR-000-2, goal spec Non-Functional Requirements | Task 2, Task 3 | Covered |
| 2 | `performCASMerge` must not depend on blackboard state or artifact-ref semantics. | NFR-000-4, goal spec Non-Functional Requirements | Task 1, Task 2 | Covered |
| 3 | Existing post-merge `ValidateArtifactRefs` backstop must remain in place. | NFR-000-5, goal spec Non-Functional Requirements | Task 1, Task 3 | Covered |
| 4 | Expose a `performCASMerge` pre-update hook accepting an optional candidate treeish callback. | Interface I-000-2, goal spec Interfaces | Task 1 | Covered |
| 5 | `performCASMerge` must accept an optional narrow pre-update hook with signature equivalent to `func(candidateTreeish string) error`. | FR-001-17, goal spec Functional Requirements | Task 1 | Covered |
| 6 | The hook must run inside each CAS attempt after candidate computation and before `UpdateRef`. | FR-001-18, goal spec Functional Requirements | Task 1, Task 3 | Covered |
| 7 | `performCASMerge` must treat the hook as opaque and own CAS retry semantics. | FR-001-19, goal spec Functional Requirements | Task 1, Task 2, Task 3 | Covered |
| 8 | The hook must be skipped for the already-merged no-op path. | FR-001-20, goal spec Functional Requirements | Task 1 | Covered |
| 9 | Fast-forward merges must validate `expectedCommit` before `UpdateRef`. | FR-001-21, goal spec Functional Requirements | Task 1 | Covered |
| 10 | True merges must validate the candidate merge treeish before `UpdateRef`. | FR-001-22, goal spec Functional Requirements | Task 1 | Covered |
| 11 | Hook failure must trigger an integration ref re-read before returning the hook error. | FR-001-23, goal spec Functional Requirements | Task 2 | Covered |
| 12 | If integration HEAD changed after candidate computation, discard stale hook error and retry. | FR-001-24, goal spec Functional Requirements | Task 2 | Covered |
| 13 | If integration HEAD is unchanged after hook failure, return the hook error. | FR-001-25, goal spec Functional Requirements | Task 2 | Covered |
| 14 | If integration HEAD cannot be re-read after hook failure, preserve hook failure and staleness-read failure. | FR-001-26, goal spec Functional Requirements | Task 2 | Covered |
| 15 | If hook passes but `UpdateRef` has a CAS conflict, retry and re-run the hook against the recomputed candidate. | FR-001-27, goal spec Functional Requirements | Task 3 | Covered |
| 16 | `performCASMerge` must not know about blackboard state, artifact-ref provenance, or artifact validation policy. | NFR-001-2, goal spec Non-Functional Requirements | Task 1, Task 2 | Covered |
| 17 | Error messages must be stable enough for tests to assert without relying on map iteration order. | NFR-001-3, goal spec Non-Functional Requirements | Task 2 | Covered |
| 18 | Given hook failure with changed integration HEAD, retry instead of returning stale hook error. | AC-001-11, goal spec Acceptance Criteria | Task 2 | Covered |
| 19 | Given hook failure and failed integration HEAD re-read, return an error preserving both failures. | AC-001-12, goal spec Acceptance Criteria | Task 2 | Covered |
| 20 | Given hook success but `UpdateRef` CAS conflict, retry and re-run the hook against the recomputed candidate. | AC-001-13, goal spec Acceptance Criteria | Task 3 | Covered |
| 21 | Given `expectedCommit` is already merged, return without running the hook. | AC-001-14, goal spec Acceptance Criteria | Task 1 | Covered |
...
The structured output file next to this plan contains one entry for each planned coding task. The `desc`, `done_when`, `scope`, `spec_ref`, `plan_ref`, and present `depends_on` fields are copied verbatim from the task sections above. There are no `task_depends_on` fields because these tasks have no dependencies on existing concrete tasks outside this output array.
