...
integration branch: 109 files, +17668/-358. Non-code files (goal spec, ADR-0113,
...
### 3. Duplicated projection evaluator and analysis role-pair mapping
...
- `prompts.evaluateEffectiveIntegrationCompletion` (`internal/prompts/wake.go:158`)
  ...
  the orchestrator dashboard from the supervisor's wake decision.
- `ops.validateIntegrationAnalysisRolePair` (`internal/ops/submit_verdict.go:462`)
  ...
  (`internal/ops/integration_reconcile.go:19`).
...
- **Frozen legacy pipelines.** `SlicedIntegrationCapability` gates only slice
  materialization (`integration_progress.go:489`) and slice prompt context, so a
  legacy `integration-pair`-only topology still reaches global analysis.
...
Three fix tasks: finding 1 (correctness, independent), finding 2 (correctness,
independent), finding 3 (code organization, sequenced after finding 2 so the
shared HEAD helper exists before the projection is consolidated onto it).
