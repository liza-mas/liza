...
Implication: Tasks 1-10 require exact passing JSON test events and reject any failing event.

[FRAGILITY]

Architectural-issue closure is a registry lifecycle transition, not the presence of evidence words anywhere after an issue heading. An unbounded scan can borrow unrelated evidence from a later issue and certify stale lifecycle state.

Implication: Task 11 extracts the named active issue, summary, completed-fix, fixed-traceability, and fix-detail sections; only an explicit evidence-bearing revision or a policy-complete evidence-bearing resolution passes, and the assertion fails on the pre-implementation registry.
