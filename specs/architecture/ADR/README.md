...
| [0068 — Optional Repository Indexing with SCIP and Stacklit](0068-optional-repository-indexing-with-scip-and-stacklit.md) | Add opt-in SCIP and Stacklit index refresh with explicit prompt paths for worktree-safe repository navigation. Project-root refresh superseded by ADR-0156. |
...
| [0136 — Validation Session Prerequisites](0136-validation-session-prerequisites.md) | Bind declared command prerequisites to the effective session context before assignment/reclaim and provider start; keep sanitized readiness as audit evidence. |
| [0137 — Selective Dependency Generation](0137-selective-dependency-generation.md) | Add optional `inherit_inputs` on `OutputEntry` so a child can wait for named upstream outputs instead of the whole phase; omitted intent keeps ADR-0048's automatic barrier. |
| [0138 — Post-Hoc Inherited Dependency Narrowing](0138-post-hoc-inherited-dependency-narrowing.md) | Orchestrator-only `narrow-inherited-dependencies` applies `inherit_inputs` after generation: persists the selection on the producer and removes unselected phase-gate edges from children still in their initial status, failing closed as a whole. |
| [0139 — Assigned-Carrier Reference Rendering](0139-assigned-carrier-reference-rendering.md) | Inline declared direct references only for the assigned carriers (parent, review, most specific scalar ref); ancestor carriers' references render as pointers to their pinned revision. |
| [0140 — Reviewer-Claim Circuit Breaker](0140-reviewer-claim-circuit-breaker.md) | Key pre-claim reviewer failures by role, task, class and boundary; quarantine deterministic failures and back off others, with one durable anomaly per key. |
| [0141 — Blocked-Assessment Idempotency](0141-blocked-assessment-idempotency.md) | Use non-assessment history counts and one shared fingerprint predicate for transactional assessment suppression and blocked-task wakes. Descendants input replaced by a declared awaited set per ADR-0157. |
| [0142 — Shared Payload Preflight Validation](0142-payload-preflight-validation.md) | Share versioned structural validators between preflight and mutation; keep preflight state-free and ungated, and require output `spec_ref`. |
| [0143 — Transactional Task Replacement](0143-transactional-task-replacement.md) | Commit replacement creation, consumer updates and supersession atomically; use source receipts for replay and validate declared preserved worktrees. |
| [0144 — Usage Attribution by Terminal Outcome](0144-usage-attribution-by-outcome.md) | Store immutable usage records outside state and derive outcomes from task history at report time. |
| [0145 — Rejection RCA Gate](0145-rejection-rca-gate.md) | Reuse BLOCKED for a verdict-boundary rejection-RCA gate, with classified dispositions and restore modes enforced by `unblock-task`. |
| [0146 — Authoritative Outcomes for Repeated Lifecycle Actions](0146-authoritative-lifecycle-outcomes.md) | Answer repeated or stale lifecycle actions from bounded durable request receipts and transition identities, and keep lifecycle state repairable with its cause exposed to the agent. |
| [0147 — Bounded Operator Notes and the Human-Note Wake](0147-bounded-operator-notes.md) | Append bounded operator notes under the state lock and deliver them through a `HUMAN_NOTE` wake trigger with per-note seen-stamping, requiring inspection before reassessment. |
| [0148 — Cached Parsed State with Per-Call Deep Copies](0148-cached-parsed-state.md) | Cache the parsed normalized state by file mtime and return a reflective deep copy per call, preserving `ReadCached`'s mutable-result contract. |
| [0149 — Per-Role Instruction Budget Measured Through the Compositor](0149-per-role-instruction-budget.md) | Measure rendered payload and instruction text per role variant through the real compositor against a committed baseline, asserting the ceiling only behind an opt-in gate flag. |
| [0150 — Shared Authoring Vocabulary: Priority, Commitments, Proof Stage](0150-shared-authoring-vocabulary.md) | Define priority, consequential-commitment and proof-stage rules at one owner, correct each conflicting instruction in place, and replace supersession-by-default with cause-based recovery. |
| [0151 — Nonblocking Inspection Snapshots and Field Queries](0151-nonblocking-inspection-queries.md) | Serve inspection from one uncached atomic snapshot per call and derive `time_in_status` from a single event classification shared by every surface. |
| [0152 — Source-Document Language Propagates to Generated Specs](0152-source-document-language.md) | Write each generated spec in the language of its assigned source material, keeping identifiers verbatim; architecture plans stay English. |
| [0153 — Documentation and ADR Conventions Elicited on a Blank Repository](0153-greenfield-documentation-conventions.md) | Elicit the shipped documentation surface and ADR convention during goal-writing when nothing exists on disk, checked by a self-suppressing readiness criterion reported as a note. |
| [0154 — Decision Records in the Universal Contract](0154-decision-records-in-core-contract.md) | CORE names decision records as durable memory and a Doc Impact category, qualified by “where the project keeps one”; `architecture-planning` owns reading prior decisions and routing candidates into the record. |
| [0155 — Spec Reconciliation Tested at Review](0155-spec-reconciliation-at-review.md) | Let either an additive-sibling or an alters-documented-behavior question falsify a Doc Impact of “none”, and have review test the declaration with severity left to reviewer judgment. |
| [0156 — Repo-Root Index Ownership by Lifecycle Hooks and the Merge Trigger](0156-repo-root-index-ownership-by-lifecycle-hooks.md) | Lifecycle Git hooks own repo-root indexes in both modes, a successful merge triggers a detached refresh, and one lock-serialized coordinator publishes atomically; the orchestrator no longer indexes and repairs the hooks at startup. |
| [0157 — Blocked-Assessment Wait-For Set](0157-blocked-assessment-wait-for-set.md) | Let a blocked assessment name the existing unfinished tasks it waits for, fingerprinting them instead of generated descendants, with a write-time deadlock guard and a wake reader that fails open on a circular set. |
