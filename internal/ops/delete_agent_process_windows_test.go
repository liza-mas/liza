...
func TestTerminateAgent_StopsSpawnedTreeBeforeRemovingState(t *testing.T) {
	...
	tree, err := procscan.ProcessTreeDeepestFirst(uint32(agent.Pid))
	if err != nil {
...
func startFakeAgentWithChild(t *testing.T) *os.Process {
	...
	stub := testhelpers.WriteShellStub(t, filepath.Join(binDir, brand.RuntimeValues().BinaryName), `#!/bin/sh
...
`)
	...
	tree, err := procscan.ProcessTreeDeepestFirst(uint32(cmd.Process.Pid))
	if err != nil {
		t.Fatalf("read process tree: %v", err)
	}
	if len(tree) < 3 {
