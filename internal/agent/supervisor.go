...
func RunSupervisor(ctx context.Context, config SupervisorConfig) error {
	...
	backgroundTaskTracker := newRuntimeFailureTracker()
	...
	for {
		...
		case 0:
			...
			// A session that exits cleanly while one of its own background jobs
			// is still live has not finished its work: the provider kills the
			// job at session end, so the completion the agent said it was
			// waiting for never arrives. Completing here would release the
			// claim and leave the worktree dirty, blocking the task on the
			// next claim. Preserve the claim and let the next session finish.
			if effectiveTask != "" {
				if abandoned := detectAbandonedBackgroundTask(currentOutput); abandoned != nil {
					handleAbandonedBackgroundTaskRetry(bb, config, effectiveTask, stateBefore.Config, *abandoned, backgroundTaskTracker, spinTracker)
					exit42Tracker.reset(taskID)
					crashTracker.reset(effectiveTask)
					continue
				}
				backgroundTaskTracker.reset(effectiveTask)
			}
