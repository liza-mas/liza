...
type ConfigureResult struct {
	...
	PowerShellEnvPath string   `json:"powershell_env_path,omitempty"`
...
func Configure(opts ConfigureOptions) (ConfigureResult, error) {
	...
	// PowerShell reads none of the POSIX profiles, so on Windows the activation
	// would be written and never applied. Emit an equivalent it can dot-source.
	goos := opts.GOOS
	if goos == "" {
		goos = runtime.GOOS
	}
	powerShellEnvPath := ""
	if goos == "windows" {
		powerShellEnvPath = filepath.Join(toolchainDir, "env.ps1")
		if err := os.WriteFile(powerShellEnvPath, []byte(renderPowerShellEnvFile(installDir, activation)), 0o644); err != nil {
			return ConfigureResult{}, fmt.Errorf("write PowerShell env: %w", err)
		}
	}
	...
	result := ConfigureResult{
		...
		EnvPath:           envPath,
		PowerShellEnvPath: powerShellEnvPath,
		InstallDir:        installDir,
...
func renderEnvFile(installDir string, activation []string) string {
	...
	fmt.Fprintf(&b, "# Generated by %s.\n", brand.Command("toolchain", "configure"))
	fmt.Fprintf(&b, "# Source this file before running %s or %s agent commands.\n", brand.Command("init"), brand.BinaryName)
	...
	if slices.Contains(activation, brand.EnvName("ENABLE_SEMBLE")+"=1") {
	...
	fmt.Fprintf(&b, "        eval \"$(%s completion bash 2>/dev/null)\"\n", brand.BinaryName)
	b.WriteString("      elif [ -n \"${ZSH_VERSION:-}\" ]; then\n")
	fmt.Fprintf(&b, "        autoload -Uz compinit 2>/dev/null || true; compinit -i 2>/dev/null || true; eval \"$(%s completion zsh 2>/dev/null)\"\n", brand.BinaryName)
	b.WriteString("      fi\n")
	b.WriteString("    fi\n")
	b.WriteString("    ;;\n")
	b.WriteString("esac\n")
...
// renderPowerShellEnvFile mirrors renderEnvFile for a shell that reads no POSIX
// profile. It is dot-sourced, not executed, so it only assigns.
func renderPowerShellEnvFile(installDir string, activation []string) string {
	var b strings.Builder
	fmt.Fprintf(&b, "# Generated by %s.\n", brand.Command("toolchain", "configure"))
	fmt.Fprintf(&b, "# Dot-source this file before running %s or %s agent commands.\n", brand.Command("init"), brand.BinaryName)
	...
	if slices.Contains(activation, brand.EnvName("ENABLE_SEMBLE")+"=1") {
		b.WriteString("# After Semble has been prewarmed and verified offline, operators may add:\n")
		b.WriteString("# $env:HF_HUB_OFFLINE = '1'\n")
	}
	return b.String()
}

// powerShellQuote wraps a value in a single-quoted PowerShell literal, where the
// only escape is a doubled quote and nothing else is expanded.
...
// process, which would let a caller that redirected home — every test here —
// write into the developer's own profile.
//
...
func appendProfileSource(profilePath, envPath string) error {
	...
	line := fmt.Sprintf("\n# %s toolchain\n[ -f %s ] && . %s\n", brand.NameTitle, quotedEnvPath, quotedEnvPath)
	existing, err := os.ReadFile(profilePath)
	if err != nil && !os.IsNotExist(err) {
